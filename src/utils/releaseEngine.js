/**
 * CineFy Realtime Movie Release Engine
...
/**
 * Known festive seasons mapped to approximate calendar dates
 */
const FESTIVE_DATES = {
...
/**
 * Parses any release date string (ISO, "August 25, 2026", "May 2027", "Diwali 2026") into a valid Date object.
 */
...
/**
 * Formats a Date object or string into a clean Indian/International display format (e.g. "25 Aug 2026")
 */
...
/**
 * Determines whether a movie has been released as of `referenceDate` (defaults to current clock time).
 * 
 * Logic:
 * - If releaseDate <= today: Released! Status becomes "now_showing"
 * - If releaseDate > today: Unreleased. Status remains "coming_soon"
 */
export function getMovieReleaseStatus(movie, referenceDate = new Date()) {
  ...
  // If no parseable release date, fallback to existing explicit status
  if (!releaseDateObj) {
...
/**
 * Enriches a single movie object with real-time dynamic release metadata and updated status.
 */
export function enrichMovieWithRealtimeRelease(movie, referenceDate = new Date()) {
  ...
  return {
    ...movie,
    // Dynamically override status based on real-time release check
...
/**
 * Enriches an array of movies with dynamic real-time release dates and provides separated categories.
 */
export function processMoviesWithRealtimeRelease(movies = [], referenceDate = new Date()) {
  ...
  // Now Showing: movies released today or in the past
  // Sorted with today's premiering movies first, then highest rating / recent releases
  const nowShowing = enriched
    .filter((m) => m.status === "now_showing")
    .sort((a, b) => {
      // 1. Released today first
      if (a.isReleasedToday && !b.isReleasedToday) return -1;
      if (!a.isReleasedToday && b.isReleasedToday) return 1;
      // 2. Then most recent release
  ...
  // Coming Soon: movies releasing in future
  // Sorted chronologically (closest release date first!)
